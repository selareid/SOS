...
function canAfford (room, cost) {
    ...
    if (energy >= cost) return 1;
        ...
        if (cost > energyCapacity || 
            (room.find(FIND_MY_CREEPS, {filter: (c) => c.memory.p && c.memory.p == 'APP'}).length < 1 && (room.find(FIND_MY_CREEPS, {filter: (c) => c.memory.p && c.memory.p == 'fillSpawn'}).length < 1
            || room.find(FIND_MY_CREEPS, {filter: (c) => c.memory.p && c.memory.p == 'fillExt'}).length < 1)) && cost > 300)  return 3;
        return 2;
    }
}
