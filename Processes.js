        ...
        run: function (Memory_it) {
            ...
            switch (Memory.n) {
                    ...
                    if (Memory.credits > 0 && _.sum(room.terminal.store) < room.terminal.storeCapacity) {
                        ...
                        if (!global.Mem.market[Memory.mineral + '_sell'] || bestBuy.price < global.Mem.market[Memory.mineral + '_sell']) {
                            ...
                            if (amountToSend > 0) {
                                ...
                                if (rsl == OK) {
                                    global.Mem.market[Memory.mineral + '_buy'] = bestBuy.price;
                ...
                case 2:
                    ...
                    while (keepGoing && count < arrayOfResourcesNeeded.length*2);
                    break;
                default:
                    Memory.n = 1;
                    if (room.terminal.store[Memory.mineral]) {
                        ...
                        if (!Memory.buyPrice || bestSell.price > Memory.buyPrice) {
                            ...
                            if (amountToSend > 0) {
                                ...
                                if (rsl == OK) {
                                    global.Mem.market[Memory.mineral + '_sell'] = bestSell.price;
        ...
        run: function (Memory_it) {
            ...
            if (creep) {
                ...
                if (Memory.w == 1) {
                    ...
                    else if (room.find(FIND_MY_SPAWNS)[Game.time % room.find(FIND_MY_SPAWNS).length].energy < SPAWN_ENERGY_CAPACITY || (creep.ticksToLive < 1250 && !room.find(FIND_MY_SPAWNS)[Game.time % room.find(FIND_MY_SPAWNS).length].spawning && room.find(FIND_MY_SPAWNS)[Game.time % room.find(FIND_MY_SPAWNS).length].energy >= 300)) {
                        if (room.find(FIND_MY_SPAWNS)[Game.time % room.find(FIND_MY_SPAWNS).length].energy >= 300) room.find(FIND_MY_SPAWNS)[Game.time % room.find(FIND_MY_SPAWNS).length].renewCreep(creep);
                        creep.transfer(room.find(FIND_MY_SPAWNS)[Game.time % room.find(FIND_MY_SPAWNS).length], RESOURCE_ENERGY);
                    }
